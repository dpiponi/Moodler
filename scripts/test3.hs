do
    ...
    container220 <- container' "panel_3x1.bmp" (-518.0,19.5) proxy203
    in221 <- plugin' (vca113 ++ "." ++ "cv") (-539.0,44.5) container220
    hide in221
    in222 <- plugin' (vca113 ++ "." ++ "signal") (-539.0,-5.5) container220
    knob223 <- knob' (input95 ++ "." ++ "result") (-539.0,44.5) container220
    rename "adsr->freqency gain" knob223
    label224 <- label' "vca" (-543.0,94.5) container220
    out225 <- plugout' (vca113 ++ "." ++ "result") (-498.0,19.5) container220
    ...
    container407 <- container' "panel_adsr.bmp" (-398.5,-300.0) proxy203
    in408 <- plugin' (adsr0 ++ "." ++ "attack") (-427.5,-239.0) container407
    hide in408
    in409 <- plugin' (adsr0 ++ "." ++ "decay") (-374.5,-222.0) container407
    hide in409
    in410 <- plugin' (adsr0 ++ "." ++ "sustain") (-441.5,-298.0) container407
    hide in410
    in411 <- plugin' (adsr0 ++ "." ++ "release") (-374.5,-322.0) container407
    hide in411
    in412 <- plugin' (adsr0 ++ "." ++ "gate") (-368.5,-371.0) container407
    knob413 <- knob' (input87 ++ "." ++ "result") (-437.5,-243.0) container407
    knob414 <- knob' (input88 ++ "." ++ "result") (-373.5,-249.0) container407
    knob415 <- knob' (input93 ++ "." ++ "result") (-444.5,-298.0) container407
    knob416 <- knob' (input92 ++ "." ++ "result") (-368.5,-301.0) container407
    out417 <- plugout' (adsr0 ++ "." ++ "result") (-368.5,-408.0) container407
    ...
    set knob184 (1.8023632)
    set knob192 (0.34185517)
    set knob210 (0.4642551)
    set knob211 (0.3222714)
    set knob217 (0.14079204)
    set knob223 (0.0)
    set knob231 (0.0)
    set knob232 (0.2827409)
    set knob277 (5.6845685e-5)
    set knob278 (0.8738309)
    set knob323 (2.8997473e-4)
    set knob324 (0.2540994)
    set knob367 (0.0)
    set knob380 (1.0e-2)
    set knob386 (0.0)
    set knob387 (0.0)
    set knob393 (0.0)
    set knob402 (4.0e-2)
    set knob403 (0.35999998)
    set knob404 (0.1258983)
    set knob405 (0.12)
    set knob413 (0.11110552)
    set knob414 (0.4731545)
    set knob415 (0.6939946)
    set knob416 (0.47777784)
    set knob426 (1.0)
    set knob449 (0.0)
    return ()
    bind '#' "sharpen"
