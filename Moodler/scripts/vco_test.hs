do
    ...
    input9  <-  new' "input"
    sum28  <-  new' "sum"
    container71 <- container' "panel_vco2.png" (x+(-84.0), y+(168.0)) root
    in72 <- plugin' (id16 ++ "." ++ "signal") (x+(-48.0), y+(204.0)) container71
    in73 <- plugin' (id7 ++ "." ++ "signal") (x+(-71.0), y+(248.0)) container71
    hide in73
    in74 <- plugin' (id8 ++ "." ++ "signal") (x+(-53.0), y+(171.0)) container71
    hide in74
    in75 <- plugin' (id10 ++ "." ++ "signal") (x+(-48.0), y+(132.0)) container71
    knob76 <- knob' (input9 ++ "." ++ "result") (x+(-48.0), y+(168.0)) container71
    knob77 <- knob' (input22 ++ "." ++ "result") (x+(-48.0), y+(240.0)) container71
    out78 <- plugout' (id15 ++ "." ++ "result") (x+(-108.0), y+(84.0)) container71
    out79 <- plugout' (id11 ++ "." ++ "result") (x+(-36.0), y+(84.0)) container71
    out80 <- plugout' (id12 ++ "." ++ "result") (x+(-108.0), y+(48.0)) container71
    out81 <- plugout' (id13 ++ "." ++ "result") (x+(-36.0), y+(48.0)) container71
    proxy82 <- proxy' (x+(-131.0), y+(250.0)) container71
    hide proxy82
    container104 <- container' "panel_3x1.png" (-815.0,439.0) proxy82
    ...
    container83 <- container' "panel_3x1.png" (-434.0,420.0) proxy82
    ...
    container88 <- container' "panel_3x1.png" (-318.0,291.0) proxy82
    ...
    container93 <- container' "panel_3x1.png" (-691.0,453.0) proxy82
    ...
    container98 <- container' "panel_3x1.png" (-826.0,199.0) proxy82
